...
**P2P 是对等网络应用体系结构，端系统之间直接通信，每个对等方既可以请求服务，也可以提供服务。**
...
视频流和 CDN 解决的是：**如何把大规模视频内容高效、稳定地传给用户。**
...
> 服务器将视频编码成多个不同码率/清晰度版本，并切成小块；客户端根据当前网络带宽和缓冲情况，动态选择合适码率的视频块下载。
...
> CDN 通过在多个地理位置部署缓存/边缘服务器，将内容放到离用户更近的位置，从而减少时延、降低源服务器压力、提高服务可用性。

工作思想：

```
用户请求视频
→ DNS/重定向等机制把用户导向合适的 CDN 节点
→ 用户从附近 CDN 节点获取内容
→ 若节点没有内容，再向源服务器或其他节点获取
```
...
|比较项|Web 缓存|CDN|
|---|---|---|
|部署位置|常在机构/ISP 代理处|大规模分布式边缘节点|
|目标|缓存 Web 对象，减少访问时延|大规模分发内容，尤其视频/静态资源|
|控制方|可能由机构/ISP 控制|通常由 CDN 服务商或内容提供商控制|
|规模|相对局部|全球/区域大规模|

**易错点**

1. CDN 不是 DNS，但可能利用 DNS 把用户导向合适节点。
2. DASH 通常基于 HTTP，不是单独替代 HTTP。
3. DASH 的自适应选择通常由客户端完成。
4. CDN 缓存的是内容，不是域名解析记录。
5. 视频流不一定要求绝对可靠，更关心连续播放体验。

**标准答题模板：CDN**

> CDN 是内容分发网络，通过在不同地理位置部署边缘服务器，将内容缓存到离用户较近的位置。用户请求内容时，系统将用户导向合适的 CDN 节点，从而减少访问时延，降低源服务器负载，减少网络流量，提高可扩展性和可用性。


---

## 使用 UDP 和 TCP 进行套接字编程 
...
|socket|作用|
|---|---|
|欢迎 socket|服务器监听客户端连接请求|
|连接 socket|accept 后生成，用于与某个客户端实际通信|

标准表达：

> TCP 服务器通常先创建欢迎 socket 监听连接请求，当客户端连接到达时，服务器调用 accept 生成新的连接 socket，用该连接 socket 与该客户端进行数据通信。

**易错点**

1. UDP 通信前不建立连接。
2. TCP 通信前必须建立连接。
3. TCP 服务器不是只用一个 socket 完成所有事，通常有欢迎 socket 和连接 socket。
4. UDP 发送时每个报文都要带目标 IP 和端口。
5. TCP 建立连接后，数据按字节流传输。