...
|层级|主要作用|数据单位 PDU|典型协议/设备|考试关键词|
|---|---|---|---|---|
|应用层|为用户应用程序提供网络服务|报文 message|HTTP、DNS、FTP、SMTP|应用协议、客户-服务器、P2P|
|传输层|为不同主机上的进程提供端到端通信|报文段 segment|TCP、UDP|端口、可靠传输、流量控制、拥塞控制|
|网络层|负责主机到主机的数据传输，选择路径并转发分组|数据报/分组 datagram/packet|IP、ICMP、路由器|IP 地址、路由、转发、子网、最长前缀匹配|
|链路层|在相邻节点之间传输数据帧|帧 frame|Ethernet、ARP、交换机|MAC 地址、成帧、差错检测、CRC、交换机|
|物理层|在物理介质上传输比特流|比特 bit|双绞线、光纤、无线信号、中继器|比特传输、信号、介质|